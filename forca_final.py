...
def verificaçãoLetra(letra,secret,secretTamanho,novosTraços,usadas,palavraSemAcentos,numeroErro,disponiveis):
    ...
    for x in range(0,qtdLetra):
            ...
            for x, i in enumerate(disponiveis): 
                if(i == letra):
                    disponiveis[x]="+"
            #guarda o index da letra, para na proxima repetição do ciclo começar no ineex+1
...
def main():
    ...
    while numeroErro<6 and traços!=secret:
        ...
        elif letra.isalpha() and len(letra)==secretTamanho and letra!=palavraSemAcentos :
            numeroErro+=1
            usadas.append(letra)
            erro(numeroErro,usadas,disponiveis,traços)
        elif not letra.isalpha() and len(letra)==secretTamanho and letra!=palavraSemAcentos:
            print("erro- introduza uma palavra de", secretTamanho,"letras ou uma letra")  
